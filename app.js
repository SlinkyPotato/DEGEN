...
// setting up points system
client.on('message', async (message) => {
  if (!message.guild || message.author.bot) return;
  if (message.content.startsWith(`${prefix}`)) return;
  
  // Insert Command filtering options based on message content
...
client.on('guildMemberAdd', (member) => {
  let embed = new MessageEmbed()
    .setTitle('Welcome!')
    .setColor(0xff0000)
    .setDescription('Have a look around and enjoy your time in the server!');
  member.send(`Hi ${member}, welcome to the BanklessDAO!`);
  member.send(embed);
  member.guild.channels.find(c => c.name === 'welcome').send(`Welcome to the party, <@${member.user.id}>`);
  member.guild.channels.find(c => c.name === 'announcements').send(`@everyone, Welcome <@${member.user.id}> to the fold!`);
});
...
client.on('raw', packet => {
  ...
  channel.messages.fetch(packet.d.message_id).then(message => { // Since we have confirmed the message is not cached, let's fetch it
