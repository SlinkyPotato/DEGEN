	...
	try {
		for (const reac of userReactions.values()) {
			...
			console.log('reaction emoji ', reac.emoji.name)
				...
				if (choice_available===true){																		//the selected choice is available, only remove choices that don't match current choice
					if (reac.emoji.name !== emoji){
						await reac.users.remove(user_id);
						console.log('reaction removed by bot, case true')
						break;
					};
				}else if (choice_available===false){															//the selected choice is not available, only remove choices that do match current choice
					if (reac.emoji.name === emoji){
						await reac.users.remove(user_id);
...
const updateUserVoteRecord = async (user_id, emoji, vote_record, message, emote_total, choice_type) => {
	switch(choice_type){
		case 'NEWVOTE': {
			let totals = updateEmoteTotals(emoji, vote_record[user_id], emote_total, choice_type)
...
const updateProgressString = (emoji, emote_total, emote_required) => {
	console.log(`updating progress string, inputs: ${emoji}, ${emote_total}, ${emote_required}`)
	let percent = Math.round(100/parseInt(emote_required[emoji])*emote_total[emoji]);
	let updateStr = `${percent}%(${emote_total[emoji]}/${emote_required[emoji]})`;
...
export default async (channel: TextChannel, request: any): Promise<any> => {
	...
	var userVoteRecord = {};  			//{user_id: emoji}

	var emoteTotal = {							//{emoji<unicode>: current_total<int>}
		[constants.EMOJIS.one] : 0,
		[constants.EMOJIS.two]: 0,
		[constants.EMOJIS.three]: 0,
	}; 						

	const emoteRequired = {			//{emoji<unicode>: required_total<int>}
		[constants.EMOJIS.one] : 1,
		[constants.EMOJIS.two]: 3,
		[constants.EMOJIS.three]: 2,
	}; 	

	var choiceAvailable = true;					
	...
	const filter = (reaction) => {
		let emoji_valid = emojiValid(reaction.emoji.name, validEmojiArray)
		console.log('emoji valid? ', emoji_valid)
		return emoji_valid;
	};

	const collector = embedMessage.createReactionCollector(filter, { dispose: true } /*{ time: 10000 }*/ );
	collector.on('collect', async (reaction, user) => {

		//check if desired choice is still available
		choiceAvailable = validateEmoteTotals(reaction.emoji.name, emoteTotal, emoteRequired)
		console.log('CHOICE AVAILABLE ', choiceAvailable)

		//compare user choice with vote record
		const choiceType = checkVoteType(user.id, reaction.emoji.name, userVoteRecord);

		switch (true) {
			case (choiceAvailable===true):{
				console.log('your choice is still available, running updateUserVoteRecord()')
				...
				if (choiceType === 'CHANGEVOTE'){
					await removeReaction(embedMessage, user.id, reaction.emoji.name, choiceAvailable);
				}
				break;
			}
			case (choiceAvailable===false):{
				...
				if (['NEWVOTE', 'REVOTE', 'CHANGEVOTE'].includes(choiceType)){
					console.log('your choice is not available, type: NEWVOTE, REVOTE or CHANGEVOTE, running removeReaction()' )
					await removeReaction(embedMessage, user.id, reaction.emoji.name, choiceAvailable);
					break;
				} else {
		...
		const newProgressString = updateProgressString(reaction.emoji.name, emoteTotal, emoteRequired)

		//deep copy embed's fields to new array
		var embedFields = Object.assign([], embedMessage.embeds[0].fields);

		//updated fields
		let updatedEmbedFields = updateEmbedFields(reaction.emoji.name, embedFields, newProgressString)
	...
	collector.on('remove', async (reaction, user) => {
		...
		if (choiceType === 'UNVOTE'){
			console.log('your choice is UNVOTE type, running updateUserVoteRecord()')
			const updateArray = await updateUserVoteRecord(
				user.id, 
				reaction.emoji.name, 
				userVoteRecord, 
