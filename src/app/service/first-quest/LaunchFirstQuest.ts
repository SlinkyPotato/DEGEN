...
export const sendFqMessage = async (dmChannel: TextBasedChannels, member: GuildMember, ): Promise<void> => {
	...
	collector.on('end', async (collected, reason) => {
		Log.debug(`Collected ${collected.size} items, reason for end ${reason}`);
...
const getMessageContentFromDb = async (): Promise<void> => {
	...
	const dbFirstQuest = await db.collection(constants.DB_COLLECTION_FIRST_QUEST).find({});
	const data = await dbFirstQuest.toArray();
...
// 		'\n' +
// 		'If you have the minimum 35,000 $BANK, follow the instructions in <#834499890569543720> to get your tags and permissions. \n' +
// 		'Once verified, you\'ll have access to all channels. Welcome aboard! Come say hello in <#838790540899254303> and join a guild from <#851864032796999744>\n' +
// 		'\n' +
...
// 		'\n' +
// 		'**To obtain a `Guest Pass`, navigate to the <#845036405667397662>  and introduce yourself, your background, what interests you in the DAO, and any skills or ways you think you might want to or be able to contribute! There are unlimited ways to contribute/participate, you don’t have to be a Shadowy Super Coder to join!**\n' +
// 		'\n' +
// 		'Once you have a Guest Pass, you\'ll have access to all channels. Welcome aboard! Come say hello in <#838790540899254303> and join a guild from <#851864032796999744>. If it feels overwhelming right now, don’t stress, just drop in to the intro channel and take it from there with guidance! \n\n' +
// 		'-----------------------------------------\n' +
// 		'\n' +
// 		'React to this message with the ✏️  emoji to access your next quest. \n',
...
// 		'Image\n',
// 	fq8: '\n**Congratulations! You have reached the end of first quest**. \n' +
// 		'\n' +
